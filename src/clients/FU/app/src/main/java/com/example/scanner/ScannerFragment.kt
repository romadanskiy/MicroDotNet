    ...
    private fun setupControls() {
        ...
        barcodeDetector.setProcessor(object : Detector.Processor<Barcode> {
            override fun release() {
                Toast.makeText(context, "Сканер был закрыт", Toast.LENGTH_SHORT)
                    .show()
            }
            ...
            override fun receiveDetections(detections: Detector.Detections<Barcode>) {
                ...
                if (barcodes.size() == 1) {
                    ...
                    getActivity()?.runOnUiThread {
                        ...
                        scanningResultViewModel.scanningResult.setValue(scannedValue);
                        var supportFragmentManager = activity?.supportFragmentManager;
                        supportFragmentManager?.beginTransaction()?.replace(R.id.fragment_container, ScanningResultFragment())?.commit();
                    }
                }else
                {
                    Toast.makeText(context, "Не удалось распознать штрих код", Toast.LENGTH_SHORT).show()
